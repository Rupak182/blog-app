...
export default async function Post({ slug ,userId}: { slug: string,userId:string| undefined }) {
    ...
    return (
        <>
            ...
            <span  className="bg-green-500 text-white p-2 rounded-xl lg:px-7 px-1 lg:text-base text-xs mt-4 font-semibold ">Author : {author?.name}</span>
            <span  className=" text-xs  p-2 rounded-xl px-7 mt-4 font-medium  ">Modified at : {post.updatedAt.toDateString()}</span>
